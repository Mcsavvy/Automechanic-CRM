...
const OrderReceipt: FC<Order> = (props) => {
  ...
  const printReceipt = () => {
    if (receiptRef.current && props.orderNo) {
      html2canvas(receiptRef.current, { scale: 1 }).then((canvas) => {
        ...
        pdf.save(`Receipt For Invoice ${formatInvoiceNumber(props.orderNo)}.pdf`);
      });
    }
  };
  useEffect(() => {
          ...
          <div className="flex items-center justify-between w-full px-6 mt-4">
            <div className="flex flex-col items-start justify-start">
              <h2 className="font-rambla font-semibold">
                Receipt For Invoice {formatInvoiceNumber(props.orderNo)}
              </h2>
              <p className="text-xs font-semibold mt-4">DATE</p>
