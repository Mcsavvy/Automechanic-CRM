...
const Overdue: FC<Partial<OverdueProps>> = ({ before, after }) => {
    ...
    return (
        <div className="max-h-[350px]  w-full flex flex-col gap-2  border border-neu-3 overflow-y-auto scrollbar-thin bg-white rounded-md shadow-inner ">
            ...
            <Link href={`/inventory/orders?order:overdue[lte]=${new Date()}`}>
                <Button variant="ghost" className="w-full flex gap-2 items-center justify-center">
                    <MoveRight />View More
                </Button>
            </Link>
        </div>
