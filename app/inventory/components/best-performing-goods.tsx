...
const BestPerformer: FC<Partial<BestPerformerProps>> = ({ before, after }) => {
  ...
  return (
    <div className="self-stretch border border-neu-3 h-full w-full bg-white rounded-md shadow-md overflow-auto">
            ...
            goods.map((good: any, idx: number) => {
              return (
                <tr key={idx} className="text-[13px] border-b border-neu-3 p-1">
                  <td className="font-rambla font-semibold sticky left-0 bg-neu-1 px-4 py-2 hover:underline hover:cursor-pointer">
                    <Link
