...
const Roles = () => {
  ...
  const populateScopes = (permissions: Record<string, boolean | string[]>) => {
    ...
    for (const [category, actions] of Object.entries(permissions)) {
      const allChecked = (Array.isArray(actions) && actions.length > 0) || actions === true;
      newScopes[category] = {
        allChecked,
        items: [
          {
            action: "",
            label: "All",
            description: "All actions in this category",
            checked: allChecked,
          },
        ],
      };
            ...
            {groups &&
              groups.map((g, i) => {
                return (
                  <li
                    key={i}
                    className="w-[700px] max-w-full flex flex-col gap-3 border-b border-b-pri-6 p-4 items-start justify-start"
                  >
